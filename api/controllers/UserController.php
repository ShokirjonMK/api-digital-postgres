...
class UserController extends ApiActiveController
{
    ...
    {
        $model = new User();
//        CAST(auth_assignment.item_name AS VARCHAR)
...
//            ->andFilterWhere(['like', 'username', Yii::$app->request->get('query')]);
        ...
        $query = $model->find()
            ...
            ->andFilterWhere(['ILIKE', 'username', Yii::$app->request->get('query')]);
...
//        ]);
    ...
    public function actionIndexOld($lang)
    {
        $model = new User();
        ...
        $pasup = Yii::$app->request->get('pasup');
        if (isset($pasup)) {
            $query->andFilterWhere([
                'in', 'users.id', [
                    732, 2704, 9526, 13657, 11715, 9022, 8878, 14297, 9643, 15060, 15069, 8912, 13174, 14710, 13933, 4415, 15070, 4696, 11627, 13237, 12460, 13063, 4676, 2932, 2913, 12466, 7569, 906, 12475, 13900, 4229, 4611, 4628, 4589, 7867, 7456, 3322, 197, 8351, 2884, 4694, 8792, 9757, 7425, 7613, 310, 4543, 189, 2889, 7492, 4675, 4605, 11939, 15071, 10211, 11880, 4279, 185
                ]
            ]);
